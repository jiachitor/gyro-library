...
(function(global, factory) {
	...
	, win = window, document = win.document, parseInt = win.parseInt, _silent = false
	...
	function _closest(el, selector, ctx) {
		if (el && ctx) {
			...
			do {
				if (
					(tag === '' || el.nodeName == tag) && (!selector.length || ((el.className + '').match(re) || []).length == selector.length)
				) {
					return el;
				}
			}
