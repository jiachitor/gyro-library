...
/*
mouseLeave(this.target,bindFunction(this,function() {
    if(this.Pause=='0'){this.scroll();};
}));
*/
...
//解释 ：Array.prototype.slice这句就是访问Array的内置方法 ,slice() 方法可从已有的数组中返回选定的元素。
...
/*
 ...
 var all_box = getElementsByClassName('',"div","formxx");
 */
...
//此方法用来获取elem这个元素下的所有元素子节点，第二个参数可选，表示指定子元素的标签名,只能获取儿子这一层，儿子之后的就不能获取
...
function getSiblings(elem) { //参数o就是想取谁的兄弟节点，就把那个元素传进去
...
/*
 例子
removeClass(currentMenu,"tab-currentMenu");
 */
...
function insertAdjacentHTML(element, swhere, shtml) {
...
function toggleClass(elem, className) {
    hasClass(elem, className) ? this.removeClass(elem, className) : this.addClass(elem, className);
};
...
removeEventListener(prev, "click",tt);
 */
...
/*
mouseOverOut(e).relatedTarget; 返回与事件的目标节点相关的节点。
 */
...
mouseMove:function(){
    ...
    bindEvent(body,"mouseover",function(e){
        e = e || window.event;
        var mousePos = mousePosition(e);
        var menu = getElementsByClassName('',"div",'i-mc');
...
window.onload = function(){
    ajax({                             //初始化 options 内的参数
        url: "b.html?" + serialize(param),
        type: "GET",
        dataType: "html",
...
/*
forEach(site_nav, function (o, index) {
...
function some(object, callback, thisp) {
    var ret = false;
    each(object,
        function() {
            if (callback.apply(thisp, arguments)) {
