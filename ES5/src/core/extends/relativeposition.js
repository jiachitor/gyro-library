/*元素定位*/
GLOBAL.Dom.RelativePosition = function() {
	...
	return function(fix, rel, options) {
		...
		options || {});
		//定义参数
		var rect = GLOBAL.Dom.clientRect(fix),
		...
		//加上滚动条
		iLeft += GLOBAL.Dom.getScrollLeft(fix);
		iTop += GLOBAL.Dom.getScrollTop(fix);
		//返回定位参数
		return {
			Left: iLeft,
			Top: iTop
